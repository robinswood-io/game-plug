...
import type { GameMessage } from "@/lib/socket-client";
...
export default function CharacterSheetPage() {
  ...
  const { joinSession, onMessage, removeMessageHandler } = useSocketContext();
  ...
  // Real-time character sync via WebSocket
  useEffect(() => {
    if (!character?.sessionId || hasJoinedSession.current) return;
    ...
    // Handler for real-time character updates
    const handleMessage = (message: GameMessage) => {
      if (message.type === 'character_updated' && message.data?.characterId === characterId) {
        // Invalidate the query to refetch character data
        queryClient.invalidateQueries({ queryKey: ["/api/characters", characterId] });

        // Show toast notification for effects
        if (message.data?.effect) {
          const effect = message.data.effect;
          toast({
            title: "Effet appliqué",
            description: `${effect.name}: ${effect.value > 0 ? '+' : ''}${effect.value}`,
          });
        }
      }
    };
  ...
  }, [character?.sessionId, character?.userId, characterId, joinSession, onMessage, removeMessageHandler, toast]);
