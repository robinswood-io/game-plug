...
function JoinSessionForm() {
  ...
  const handleJoinSession = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/sessions/join/${sessionCode.toUpperCase()}`, {
        credentials: 'include',
      });
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-deep-black flex items-center justify-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-aged-gold"></div>
      </div>
    }>
      <JoinSessionForm />
    </Suspense>
