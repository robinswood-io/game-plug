...
export default function GameBoard() {
  ...
  // States
  const [projectionContent, setProjectionContent] = useState<ProjectionContent>(
    { type: 'none' }
  );
  ...
  const getCharacterStatusColor = (character: CharacterWithDetails) => {
    ...
    if (hpPercent <= 0.3 || sanityPercent <= 0.3)
      return 'border-blood-burgundy bg-blood-burgundy/10';
    if (hpPercent <= 0.6 || sanityPercent <= 0.6)
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-deep-black flex items-center justify-center">
        <div className="text-blood-burgundy font-cinzel text-2xl">
          Session introuvable
        </div>
      </div>
    );
                    ...
                    >
                      <CardContent className="p-2">
                        ...
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          {(character.hitPoints <=
                            character.maxHitPoints * 0.3 ||
                            character.sanity <=
                              character.maxSanity * 0.3) && (
                          ...
                          <div className="space-y-0.5">
                            <div className="flex justify-between items-center">
                              <div className="flex items-center gap-0.5">
                                ...
                                <span className="text-[10px] text-aged-parchment">
                                  PV
                                </span>
                              </div>
                          ...
                          <div className="flex items-center justify-between bg-cosmic-void/30 rounded px-1 py-0.5">
                            ...
                            <span className="text-[10px] font-bold text-yellow-500">
                              {typeof character.money === 'string'
                                ? parseFloat(character.money).toFixed(0)
                                : (character.money || 0).toFixed(0)}
                            </span>
                          </div>
                        </div>

                        {/* Active Effects - Only show critical ones */}
                        {(character.sanityConditions.length > 0 ||
                          character.activeEffects.length > 0) && (
                          <div className="flex flex-wrap gap-1">
                            {character.sanityConditions.slice(0, 1).map(
                              (condition) => (
                                <Badge
                                  key={condition.id}
                                  variant="outline"
                            ...
                            {character.activeEffects.slice(0, 1).map(
                              (effect) => (
                                <Badge
                                  key={effect.id}
                                  variant="outline"
                                  className={cn(
                                    ...
                                    effect.type === 'buff'
                                      ? 'bg-eldritch-green/20 border-eldritch-green'
                                      : 'bg-blood-burgundy/20 border-blood-burgundy'
                                  )}
                                >
                                  {effect.type === 'buff' ? '↑' : '↓'}{' '}
                                  {effect.name}
          ...
          <div className="bg-charcoal border-b border-aged-gold/30 p-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-cinzel text-aged-gold">
                  Zone de Projection
                </span>
              </div>
        ...
        <div className="flex-1 bg-deep-black relative overflow-hidden">
          {projectionContent.type === 'none' ? (
            <div className="h-full flex flex-col items-center justify-center text-aged-parchment">
              ...
              <h3 className="text-2xl font-cinzel mb-2">
                Zone de Projection
              </h3>
              <p className="text-center max-w-md">
                Utilisez les outils ci-dessous pour projeter des images,
                descriptions et éléments visuels aux joueurs.
              </p>
            </div>
