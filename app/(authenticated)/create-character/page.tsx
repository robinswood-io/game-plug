...
type InsertCharacter = Partial<Omit<Character, 'id' | 'createdAt' | 'updatedAt'>> & {
  name: string;
  occupation: string;
  sessionId: string;
  strength: number;
...
export default function CreateCharacterPage() {
  ...
  // Check if coming from a session join flow (client-side only)
  const [sessionFromStorage, setSessionFromStorage] = useState<string>("");
  const [sessionIdFromStorage, setSessionIdFromStorage] = useState<string>("");

  useEffect(() => {
    if (typeof window !== 'undefined') {
      setSessionFromStorage(localStorage.getItem('createCharacterForSession') || "");
      setSessionIdFromStorage(localStorage.getItem('currentSessionId') || "");
    }
  }, []);
  ...
  const { data: sessions = [], isLoading: sessionsLoading } = useQuery<GameSession[]>({
    ...
    retry: false,
    enabled: !sessionFromStorage && !sessionIdFromStorage,
  });
  ...
  const form = useForm<CharacterCreationForm>({
    ...
    defaultValues: {
      ...
      residence: "",
      gender: "",
      sessionId: sessionFromStorage || sessionIdFromStorage || "",
      avatarDescription: "",
    },
  });
  ...
  const onSubmit = (data: CharacterCreationForm) => {
    ...
    const characterData: InsertCharacter = {
      ...
      height: physicalTraits.height || null,
      build: physicalTraits.build || null,
      hairColor: physicalTraits.hairColor || null,
      eyeColor: physicalTraits.eyeColor || null,
      ...
      avatarPrompt: buildDescription() || null,
      isActive: true,
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid md:grid-cols-3 gap-3 max-h-96 overflow-y-auto pr-2">
                        {Object.entries(skillPoints)
                          .filter(([key, value]) => value > (DEFAULT_SKILLS[key] || 0) || allocatedPoints[key])
                          .sort(([keyA], [keyB]) => {
                            const nameA = SKILL_TRANSLATIONS[keyA] || keyA.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
                            const nameB = SKILL_TRANSLATIONS[keyB] || keyB.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
                            return nameA.localeCompare(nameB, 'fr');
