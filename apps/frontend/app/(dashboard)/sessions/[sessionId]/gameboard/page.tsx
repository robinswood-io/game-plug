...
import {
  Users, Monitor, Heart, Brain, Shield, Coins, Eye, EyeOff,
  Maximize, Minimize, RotateCcw, Skull, Sparkles
} from "lucide-react";
...
export default function GameBoard() {
  ...
  useEffect(() => {
    if (isConnected && sessionId) {
      ...
      console.log('✅ GameBoard joined session:', sessionId);
    }

    return () => {
      if (isConnected && sessionId) {
        sendMessage('leave_session', { sessionId });
      }
    };
                      ...
                      <CardContent className="p-2">
                        ...
                        {(character.sanityConditions.length > 0 || character.activeEffects.length > 0) && (
                          <div className="flex flex-wrap gap-1">
                            {character.sanityConditions.slice(0, 1).map((condition: any) => (
                              <Badge
                                key={condition.id}
                                variant="outline"
                            ...
                            {character.activeEffects.slice(0, 1).map((effect: any) => (
                              <Badge
                                key={effect.id}
                                variant="outline"
                                className={cn(
