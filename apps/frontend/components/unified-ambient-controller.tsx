...
interface AmbientSound {
  id: string;
  name: string;
  category: 'atmosphere' | 'pathname' | 'event';
  icon: any;
  description: string;
  mood: string;
...
export default function UnifiedAmbientController({ 
  ...
  const [selectedCategory, setSelectedCategory] = useState<'all' | 'atmosphere' | 'pathname' | 'event'>('all');
      ...
      <CardContent className="space-y-4">
        ...
        <Tabs defaultValue="all" onValueChange={(v) => setSelectedCategory(v as any)}>
          <TabsList className="grid w-full grid-cols-4 bg-gray-800">
            ...
            <TabsTrigger value="pathname">Lieux</TabsTrigger>
