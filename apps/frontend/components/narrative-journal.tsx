...
export function NarrativeJournal({ sessionId }: NarrativeJournalProps) {
  ...
  const [entryType, setEntryType] = useState<"note" | "event" | "npc" | "pathname" | "clue">("note");
  ...
  const getAiSuggestionMutation = useMutation({
    mutationFn: async (context: string) => {
      const response = await AiService.aiControllerSuggestNarrative({
        sessionId,
        context,
      });
      return response;
  ...
  const getEntryTypeIcon = (type: string) => {
    switch (type) {
      ...
      case "pathname": return <MapPin className="h-3 w-3" />;
      ...
      <CardContent>
        <Tabs value={entryType} onValueChange={(v) => setEntryType(v as any)} className="w-full">
          <TabsList className="grid grid-cols-5 mb-4 bg-cosmic-void">
            ...
            <TabsTrigger value="pathname" className="data-[state=active]:bg-aged-gold data-[state=active]:text-cosmic-void">
